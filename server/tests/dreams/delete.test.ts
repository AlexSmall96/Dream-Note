...
test('User cannot delete dream they are not the owner of.', async () => {
    ...
    const response = await request(server).delete(`${url}/${oldDreamId}`).set(...userOneAuth).expect(403)
    assertSingleError(response.body.errors, 'You are not authorized to delete this dream.')
})
