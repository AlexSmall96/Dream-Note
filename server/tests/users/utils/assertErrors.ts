...
export const assertSingleError = (
    errorsResponse: {param?: string, msg: string}[], msg: string, param?: string
) => {
    expect(errorsResponse).toHaveLength(1)
    expect(errorsResponse[0]).toMatchObject(param? {msg, param}: {msg})
}